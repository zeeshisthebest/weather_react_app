...
import SkeletonOtherLocationCard from "./SkeletonOtherLocationCard";
...
const OtherLocationCard = ({ loc, onClick }) => {
    ...
    const getWeather = useCallback(async () => {
        try {
            const resp = await currentWeatherService.getCurrentWeather(loc);
            const astro = await currentWeatherService.getAstroData(loc);
            const mdl = currentWeatherService.mapToRecentModel(resp.data, astro.data);
            setWeatherState(mdl);
        } catch (error) {
    ...
    if (!wthr) {
        return <SkeletonOtherLocationCard />;
    }
